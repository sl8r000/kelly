...
Let's say that the market has observed $n$ coin flips, $a$ of which were heads and $b$ of which were tails. If the market has correctly incorporated this information, then the market "thinks" that $p \sim \\text{Beta}(a+1, b+1)$ and "believes" that the next flip $X_{n+1}$ is is a Bernoulli trial with probability $p$ of heads. Letting $\hat{p} = (a+1)/(a+b+2)$, the price of going long will be $\hat{p} \cdot \text{$}100$ and the price of going short will be $(1 - \hat{p}) \cdot \text{$}100$.
